CREATE TABLE sepolia.fct_head_first_seen_by_node_local
(
    ...
    `seen_slot_start_diff` UInt32 COMMENT 'The time from slot start for the client to see the head event' CODEC(DoubleDelta, ZSTD(1)),
    `block_root` String COMMENT 'The head block root hash' CODEC(ZSTD(1)),
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/{installation}/{cluster}/tables/{shard}/mainnet/fct_head_first_seen_by_node_local', '{replica}', updated_date_time)
PARTITION BY toStartOfMonth(slot_start_date_time)
ORDER BY (slot_start_date_time, block_root, meta_client_name)
SETTINGS deduplicate_merge_projection_mode = 'rebuild', index_granularity = 8192
COMMENT 'When the head event was first seen on the network by a sentry node'
