CREATE TABLE default.libp2p_identify_local
(
    ...
    `success` Bool COMMENT 'Whether the identify request succeeded' CODEC(ZSTD(1)),
    ...
    `protocols` Array(String) COMMENT 'Protocols supported by the remote peer' CODEC(ZSTD(1)),
    `listen_addrs` Array(String) COMMENT 'Listen addresses reported by the remote peer' CODEC(ZSTD(1)),
    `observed_addr` String COMMENT 'Address the remote peer observed for the local node' CODEC(ZSTD(1)),
    `transport` LowCardinality(String) COMMENT 'Transport used for the connection',
    `security` LowCardinality(String) COMMENT 'Security protocol used for the connection',
    `muxer` LowCardinality(String) COMMENT 'Multiplexer used for the connection',
    `direction` LowCardinality(String) COMMENT 'Connection direction',
    `remote_multiaddr` String COMMENT 'Multiaddress of the remote peer' CODEC(ZSTD(1)),
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/{installation}/{cluster}/default/tables/libp2p_identify_local/{shard}', '{replica}', updated_date_time)
