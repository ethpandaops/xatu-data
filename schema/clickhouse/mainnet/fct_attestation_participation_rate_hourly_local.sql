CREATE TABLE mainnet.fct_attestation_participation_rate_hourly_local
(
    ...
    `moving_avg_participation_rate` Float32 COMMENT 'Moving average participation rate (6-hour window)' CODEC(ZSTD(1))
)
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/{installation}/{cluster}/tables/{shard}/mainnet/fct_attestation_participation_rate_hourly_local', '{replica}', updated_date_time)
PARTITION BY toStartOfMonth(hour_start_date_time)
ORDER BY hour_start_date_time
SETTINGS index_granularity = 8192
COMMENT 'Hourly aggregated attestation participation rate statistics with percentiles, Bollinger bands, and moving averages'
