CREATE TABLE mainnet.fct_blob_count_hourly_local
(
    `updated_date_time` DateTime COMMENT 'Timestamp when the record was last updated' CODEC(DoubleDelta, ZSTD(1)),
    `hour_start_date_time` DateTime COMMENT 'Start of the hour period' CODEC(DoubleDelta, ZSTD(1)),
    `block_count` UInt32 COMMENT 'Number of slots with blobs in this hour' CODEC(ZSTD(1)),
    `total_blobs` UInt64 COMMENT 'Total blobs in this hour' CODEC(ZSTD(1)),
    ...
    `moving_avg_blob_count` Float32 COMMENT 'Moving average blob count (6-hour window)' CODEC(ZSTD(1))
)
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/{installation}/{cluster}/tables/{shard}/mainnet/fct_blob_count_hourly_local', '{replica}', updated_date_time)
PARTITION BY toStartOfMonth(hour_start_date_time)
ORDER BY hour_start_date_time
SETTINGS index_granularity = 8192
COMMENT 'Hourly aggregated consensus layer blob count statistics with percentiles, Bollinger bands, and moving averages'
