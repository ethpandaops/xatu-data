CREATE TABLE mainnet.fct_block_data_column_sidecar_first_seen_by_node_local
(
    ...
    `seen_slot_start_diff` UInt32 COMMENT 'The time from slot start for the client to see the data column' CODEC(DoubleDelta, ZSTD(1)),
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/{installation}/{cluster}/tables/{shard}/mainnet/fct_block_data_column_sidecar_first_seen_by_node_local', '{replica}', updated_date_time)
PARTITION BY toStartOfMonth(slot_start_date_time)
ORDER BY (slot_start_date_time, block_root, column_index, meta_client_name)
