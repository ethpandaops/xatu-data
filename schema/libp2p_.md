...
- **mainnet**: `2024-05-01` to `2026-02-03`
- **holesky**: `2024-05-01` to `2025-10-26`
- **sepolia**: `2024-05-01` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_gossipsub_beacon_attestation/2026/2/3/0.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains beacon block messages received via libp2p gossipsub. Collected from deep instrumentation within forked consensus layer clients (Prysm/Lighthouse). Each row represents a block gossiped on the p2p network with timing and peer metadata. Partition: monthly by `slot_start_date_time`.
...
- **sepolia**: `2024-04-26` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_gossipsub_beacon_block/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
- **mainnet**: `2025-12-03` to `2026-02-03`
- **holesky**: `2025-10-01` to `2025-10-26`
- **hoodi**: `2025-10-28` to `2026-02-03`
- **sepolia**: `2025-10-14` to `2026-02-03`
...
> https://data.ethpandaops.io/xatu/NETWORK/databases/default/libp2p_gossipsub_data_column_sidecar/YYYY/MM/DD.parquet
```bash
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_gossipsub_data_column_sidecar/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
| **propagation_slot_start_diff** | `UInt32` | *Difference in slot start time for propagation* |
| **proposer_index** | `UInt32` | *The proposer index of the beacon block* |
| **column_index** | `UInt64` | *Column index associated with the record* |
...
- **mainnet**: `2025-07-11` to `2026-02-03`
- **holesky**: `2025-07-11` to `2025-10-26`
- **sepolia**: `2025-07-11` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_gossipsub_aggregate_and_proof/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains CONNECTED events when connections are established to remote peers. Collected from deep instrumentation within forked consensus layer clients. Each row includes remote peer agent info and geolocation. Partition: monthly by `event_date_time`.

### Availability
Data is partitioned **daily** on **event_date_time** for the following networks:

- **mainnet**: `2024-04-24` to `2026-02-03`
- **hoodi**: `2025-03-17` to `2026-02-03`
- **sepolia**: `2024-04-22` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_connected/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_disconnected/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_add_peer/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_remove_peer/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
- **mainnet**: `2025-05-30` to `2026-02-03`
- **hoodi**: `2025-05-29` to `2026-02-03`
- **sepolia**: `2025-05-29` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_recv_rpc/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_send_rpc/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains RPC messages dropped (not processed) by the peer. Collected from deep instrumentation within forked consensus layer clients. Partition: monthly by `event_date_time`.
...
- **mainnet**: `2025-05-30` to `2026-02-03`
- **hoodi**: `2025-05-29` to `2026-02-03`
- **sepolia**: `2025-05-30` to `2025-12-22`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_drop_rpc/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
- **mainnet**: `2024-05-01` to `2026-02-03`
- **hoodi**: `2025-03-17` to `2026-02-02`
- **sepolia**: `2024-05-01` to `2026-01-27`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_join/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
- **mainnet**: `2025-06-01` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_leave/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains GRAFT events when a peer joins the mesh for a topic. Collected from deep instrumentation within forked consensus layer clients. Tracks mesh membership changes. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_graft/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains PRUNE events when a peer is removed from the mesh for a topic. Collected from deep instrumentation within forked consensus layer clients. Tracks mesh membership changes. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_prune/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains DUPLICATE_MESSAGE events when a message is received that was already seen. Collected from deep instrumentation within forked consensus layer clients. Useful for analyzing message propagation. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_duplicate_message/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_deliver_message/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains metadata protocol handling events (req/resp). Collected from deep instrumentation within forked consensus layer clients. Each row represents a metadata exchange with a peer including their attnets and syncnets. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_handle_metadata/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_handle_status/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_control_ihave/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains IWANT control messages from gossipsub. Collected from deep instrumentation within forked consensus layer clients. Peers request specific message IDs. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_control_iwant/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_control_idontwant/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_control_graft/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains PRUNE control messages from gossipsub RPC. Collected from deep instrumentation within forked consensus layer clients. Peers are removed from the mesh for a topic. Partition: monthly by `event_date_time`.

### Availability
Data is partitioned **daily** on **event_date_time** for the following networks:

- **mainnet**: `2025-07-02` to `2026-02-03`
- **hoodi**: `2025-06-23` to `2026-02-03`
- **sepolia**: `2025-06-23` to `2026-02-03`
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_control_prune/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
Contains RPC subscription changes from gossipsub. Collected from deep instrumentation within forked consensus layer clients. Each row represents a subscribe/unsubscribe action for a topic. Partition: monthly by `event_date_time`.
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_subscription/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_meta_message/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
- **mainnet**: `2025-12-03` to `2026-02-03`
- **hoodi**: `2025-12-03` to `2026-02-03`
- **sepolia**: `2025-12-03` to `2026-02-03`
...
> https://data.ethpandaops.io/xatu/NETWORK/databases/default/libp2p_rpc_data_column_custody_probe/YYYY/MM/DD.parquet
```bash
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_rpc_data_column_custody_probe/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
docker run --rm -it --net host clickhouse/clickhouse-server clickhouse client --query="""
    SELECT
        *
    FROM default.libp2p_rpc_data_column_custody_probe FINAL
    WHERE
        event_date_time >= NOW() - INTERVAL '1 HOUR'
    LIMIT 10
    FORMAT Pretty
...
| **column_index** | `UInt64` | *Column index being probed* |
| **column_rows_count** | `UInt16` | *Number of rows in the column* |
| **beacon_block_root** | `FixedString(66)` | *Root of the beacon block* |
| **peer_id_unique_key** | `Int64` | *Unique key associated with the identifier of the peer* |
| **result** | `LowCardinality(String)` | *Result of the probe* |
| **response_time_ms** | `Int32` | *Response time in milliseconds* |
| **error** | `Nullable(String)` | *Error message if probe failed* |
| **meta_client_name** | `LowCardinality(String)` | *Name of the client that executed the probe* |
...
Contains periodic heartbeat snapshots of libp2p peer state. Collected from deep instrumentation within forked consensus layer clients. Each row contains mesh/peer counts and topic subscriptions at that moment. Partition: monthly by `event_date_time`.

### Availability
Data is partitioned **daily** on **event_date_time** for the following networks:

- **mainnet**: `2025-08-21` to `2026-02-03`
- **holesky**: `2025-09-29` to `2025-10-20`
- **hoodi**: `2025-08-21` to `2026-02-03`
- **sepolia**: `2025-08-21` to `2026-02-03`
...
> https://data.ethpandaops.io/xatu/NETWORK/databases/default/libp2p_synthetic_heartbeat/YYYY/MM/DD.parquet
```bash
docker run --rm -it clickhouse/clickhouse-server clickhouse local --query --query="""
    SELECT
        *
    FROM url('https://data.ethpandaops.io/xatu/mainnet/databases/default/libp2p_synthetic_heartbeat/2026/2/3.parquet', 'Parquet')
    LIMIT 10
    FORMAT Pretty
"""
```
...
|--------|------|-------------|
| **updated_date_time** | `DateTime` | *Timestamp when the record was last updated* |
| **event_date_time** | `DateTime64(3)` | *Timestamp of the heartbeat event* |
| **remote_peer_id_unique_key** | `Int64` | *Unique key of the remote peer* |
| **remote_maddrs** | `String` | *Multiaddress of the remote peer* |
| **latency_ms** | `Nullable(Int64)` | *EWMA latency in milliseconds (0 if unavailable)* |
| **direction** | `LowCardinality(String)` | *Connection direction (Unknown/Inbound/Outbound)* |
| **protocols** | `Array(String)` | *List of supported protocols* |
| **connection_age_ms** | `Nullable(Int64)` | *Connection age in milliseconds* |
| **remote_agent_implementation** | `LowCardinality(String)` | *Implementation of the remote peer* |
| **remote_agent_version** | `LowCardinality(String)` | *Version of the remote peer* |
| **remote_agent_version_major** | `LowCardinality(String)` | *Major version of the remote peer* |
| **remote_agent_version_minor** | `LowCardinality(String)` | *Minor version of the remote peer* |
| **remote_agent_version_patch** | `LowCardinality(String)` | *Patch version of the remote peer* |
| **remote_agent_platform** | `LowCardinality(String)` | *Platform of the remote peer* |
| **remote_ip** | `Nullable(IPv6)` | *IP address of the remote peer* |
| **remote_port** | `Nullable(UInt16)` | *Port of the remote peer* |
| **remote_geo_city** | `LowCardinality(String)` | *City of the remote peer* |
| **remote_geo_country** | `LowCardinality(String)` | *Country of the remote peer* |
| **remote_geo_country_code** | `LowCardinality(String)` | *Country code of the remote peer* |
| **remote_geo_continent_code** | `LowCardinality(String)` | *Continent code of the remote peer* |
| **remote_geo_longitude** | `Nullable(Float64)` | *Longitude of the remote peer* |
| **remote_geo_latitude** | `Nullable(Float64)` | *Latitude of the remote peer* |
| **remote_geo_autonomous_system_number** | `Nullable(UInt32)` | *ASN of the remote peer* |
| **remote_geo_autonomous_system_organization** | `Nullable(String)` | *AS organization of the remote peer* |
| **meta_client_name** | `LowCardinality(String)` | *Name of the client that collected the data. The table contains data from multiple clients* |
| **meta_client_id** | `String` | *Unique Session ID of the client* |
| **meta_client_version** | `LowCardinality(String)` | *Version of the client* |
| **meta_client_implementation** | `LowCardinality(String)` | *Implementation of the client* |
